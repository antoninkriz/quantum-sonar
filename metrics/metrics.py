...
class Metrics:
    def __init__(self, val_curr, val_real):
        ...
        self.TP = np.sum((self.val_curr > 0.5) & (self.val_real > 0.5))
        self.TN = np.sum((self.val_curr < 0.5) & (self.val_real < 0.5))
        self.FP = np.sum((self.val_curr > 0.5) & (self.val_real < 0.5))
        self.FN = np.sum((self.val_curr < 0.5) & (self.val_real > 0.5))
